...
We can select the top 25 variables based on higher standard deviations.
```{r}
sds <- apply((train %>% select(-label)), 2, sd) %>% sort(decreasing=T)
topvars <- sds %>% head(25) %>% names
print(topvars)
```

Next, we can remove the variables with higher correlation. Here, we drop the variables with absolute correlation greater than 0.70 with any other of the other variables.

```{r}
to_remove <- train %>% 
              select(one_of(topvars)) %>% 
                cor(.) %>% 
                  findCorrelation(.,cutoff = .70, verbose=T, names=T)

print(to_remove)
```

After removing the higher correlated variables, we can see our final set of features.
```{r}
...
svmFitLin <- fitting("svmLinear",tune.grid,topvars)
```
...
svmFitRad <- fitting("svmRadial",tune.grid,topvars)
```

### 4.2.3 - Dimensionality reduction: t-SNE
...
### 4.3.1 - Feature selection: regularized model
...
### 4.3.2 - Regularization parameter selection
...
Additionally, if we would like to give some interpretation to the variables used in the section **4.1.1**, we can see how a logistic regression model perform using this set of variables. 

```{r}
topvars_model <- fitting_glm(topvars)
```
